...
3. Register this exact callback URL:
   ...
   ```text
   http://localhost:8766/auth/x/callback
   ```
...
X_REDIRECT_URI=http://localhost:8766/auth/x/callback
```
...
Manual OAuth, `whoami`, preview, and the first controlled real post require
the account owner. Do not publish until the exact payload and account shown by
preview have been reviewed.
